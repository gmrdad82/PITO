...
# (:games_search shell). Single component, mode-aware, wraps the
...
# thumbnails + the IGDB-overwrite confirm modal trigger).
# The dispatcher branch lives here so the controller can call the
# component uniformly across all modes.
#
# Args:
#   mode:    one of :games_search. (:game_index delegates to legacy partial)
#   query:   sanitized query string (or blank when input is empty).
#   result:  Game::SearchService::Result struct for :games_search. For
#             :game_index it is a Hash with :results (IGDB rows) and
#             :search_error (string or nil) keys — passed through to
#             the legacy partial.
module Search
  class OmnisearchResultsComponent < ViewComponent::Base
    ...
    # renders directly. This component covers only the `:games_search`
    # omnisearch surface.
    MODES = %i[games_search].freeze

    FRAME_IDS = {
      games_search: "omnisearch_results_games_search"
    }.freeze

    def initialize(mode:, query:, result:)
