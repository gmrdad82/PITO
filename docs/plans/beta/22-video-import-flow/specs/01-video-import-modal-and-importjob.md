...
> Introduce `[import]` on `/videos`. First-class affordance, NOT a keybinding-
> only feature. Click opens a modal; the modal walks channel selection →
> per-channel background jobs → live progress → keep/reject confirmation
> table. Rejected videos land in a `RejectedVideoImport` tombstone so the
> daily sync never re-imports them. Source of truth: B6 of
...
This is the videos-side counterpart to the existing channel sync framework.
It is NOT the daily sync (Phase 22's sibling work) and it does NOT push to
YouTube — it only pulls metadata for newly-discovered uploads.
...
- Real YouTube API wiring. The `Channels::VideoImporter` service has a single
  injection point for the upstream call; tests stub it. Live wiring is the
  Phase 7/8 territory.
...
- `config/routes.rb`:
  ...
  ```
- Friendly URL slugs preserved on the channel-side references via
  `FriendlyId` (consistent with existing channel routes).
...
- `app/services/channels/video_importer.rb` (new) — encapsulates the
  `playlistItems.list` walk, the diff against `videos` + `rejected_video_
  imports`, the row creation, and the counter updates. Single public method
  `call(channel:, import_job:)`. Yields per-page progress via a block so the
  job can broadcast Turbo Streams without the service knowing about Action
  Cable.
...
- `app/views/videos/index.html.erb` — add `[import]` button alongside existing
  controls, just-near the `[add]` affordance restored in B1.
- `app/views/channels/show.html.erb` — add in-flight-import badge / link that
  reopens the modal at the progress step.
- `app/components/imports/progress_indicator_component.rb` +
...
- `config/locales/en.yml` — labels for `[import]`, `[start import]`,
  `[keep]`, error copy ("an import is already running for this channel"),
  modal headings, progress lead paragraph (one sentence per line per
  architect rule B).
...
- No project-doc changes expected. If the spec introduces a new component
  ...
  existing `=---` indicator), the architect-docs agent updates
  `docs/design.md` after the user validates.
...
| `enqueued_by_id`    | bigint    | FK → users.id, NOT NULL             |
...
- `has_many :imported_video_records` — derived via the `Video#import_job_id`
  back-reference? No — we deliberately do NOT add a foreign key from `videos`
  to `import_jobs`. The `imported_videos` counter is the canonical answer;
  videos created by an import live independently after the import finishes.
  This keeps `Video#destroy` cheap and avoids `dependent: :destroy` cascade
  surprises.
...
| `rejected_by_user_id`  | bigint    | FK → users.id, NOT NULL              |
| `created_at`           | timestamp | NOT NULL                             |
| `updated_at`           | timestamp | NOT NULL                             |
...
- **Indexability.** The diff query at import time is
  ...
  sync.
- **Concurrency.** Two parallel jobs (or a job + a user-initiated keep/reject
  confirm running in different requests) racing to insert the same rejection
  collide cleanly on the unique index. A jsonb-array update has to read,
  mutate, write — which means either a transaction with `SELECT FOR UPDATE`
  or a race window.
- **Auditability.** `rejected_at` + `rejected_by_user_id` give a real audit
  trail. A jsonb array of bare strings loses that without nested objects,
  which then defeats the indexability argument anyway.
...
the same transaction so the tombstone is durable before the destroy completes.
...
the job. This is a general policy beyond this spec, but the spec calls it
out explicitly for `Channel::ImportVideosJob` and any job we touch in passing
(audit log here is mandatory, but global enforcement is a follow-up phase).
...
`Imports::ChannelsController` (note: channels-plural, but the resource is
"channel-import-job"; the name reads as "imports of channels") because the
selection step lists channels and the routes thread reads cleanly. A future
`Imports::VideosController` could exist for direct video imports without
hurting the namespace.
...
JSON returns the same shape via jbuilder, with `connected` and
`in_flight` serialized as `"yes"`/`"no"`.
...
- Per-user 5-second cache lock on the action — `Rails.cache.write(
  "imports:enqueue:user:#{current_user.id}", true, expires_in: 5.seconds,
  unless_exist: true)`. If the lock is already set, respond 429 with the
  bracketed-link copy "[try again in a moment]".
- For each `channel_id`:
  ...
  3. Create an `ImportJob` in `queued` status with `enqueued_by:
     current_user`.
  4. Enqueue `Channel::ImportVideosJob.perform_later(channel.id,
     import_job.id)`.
...
- Renders the progress step for that one job, OR the keep/reject table if
...
- JSON returns the full job payload + (if `completed`) the list of imported
...
- Resolve the candidate set: all videos newly created by this `ImportJob`
  (derived from the controller's view of "videos for this channel created
  during the job's window between `started_at` and `completed_at`" — the
  selector is encapsulated in `ImportJob#candidate_videos`).
- In a single transaction:
  - For every candidate video NOT in `keep_video_ids`:
    - Insert a `RejectedVideoImport(channel:, youtube_video_id:,
...
operations with a one-element id list (`/<action>s/:type/:ids`). The keep/
reject confirm IS a bulk destructive action (deleting N videos at once).
The controller's `update` action lives at `/imports/channels/:id` because the
...
single transaction. This does NOT route through `DeletionsController`
because the destruction is a side-effect of confirming the import, not the
...
flow: keep/reject form → action confirmation page → destroy. The default is
to keep it in-modal because the user already implicitly confirmed by
...
class Channel::ImportVideosJob < ApplicationJob
  ...
  def perform(channel_id, import_job_id)
    ...
    Channels::VideoImporter.new.call(channel:, import_job:) do |page_progress|
      import_job.update!(
        total_videos:    page_progress.total,
        imported_videos: page_progress.imported,
      )
  ...
  def broadcast_progress(import_job)
    Turbo::StreamsChannel.broadcast_replace_to(
      [import_job, :progress],
      target:  helpers.dom_id(import_job, :progress),
...
(Snippet is illustrative — the spec dictates intent, the rails-impl agent
writes final code.)
...
1. Resolve the channel's uploads playlist (a YouTube API concept;
   pre-OAuth-phase this is a stubbed seam returning a fixture id).
2. Page through `playlistItems.list` (50 per page).
3. For each page:
   - Diff against existing `Video.where(channel: channel,
     youtube_video_id: ids_in_page)`.
   - Diff against `RejectedVideoImport.where(channel: channel,
     youtube_video_id: ids_in_page)`.
...
- Permanent (404 channel, malformed payload, missing uploads playlist) →
  raise `Channels::VideoImporter::FatalError` with `code:` + `suppress_retry:
  true`.
...
- New `[import]` bracketed link in the page header controls, alongside the
  existing `[add]` (restored in B1 of the same Mobile note) and `[bulk
  delete]` affordances. Order: `[add] [import] [bulk delete]`.
- Bracketed-link convention: `[import]` — no inner padding spaces (per
  architect rule A).
- Clicking opens the modal via a Turbo Frame request to
  `GET /imports/channels`. The modal is the existing wide variant (`.modal--
  wide` or whatever the Phase 4 modal primitive declares) per the
  keybindings note D23.
...
- A list of connected channels. Each row:
  - `[ ] channel-slug — channel label` (the `[ ]` here is the 3-char
    checkbox indicator per architect rule A note about the checkbox
    exception).
  - If the channel has an in-flight ImportJob, the row shows the
    in-flight badge: `[ ] channel-slug — channel label  [import running]`
    with the checkbox disabled and `[import running]` linking to
    `/imports/channels/:id`.
...
- When all jobs reach a terminal state (`completed` or `failed`), the modal
  ...
  - Default proposed behavior: each `ImportJob` swaps to its own keep/reject
...
- **Yes/no boundary.** Every external boolean — `connected`, `in_flight`,
  `keep_video_ids` truthy flags if any sneak in, MCP payloads if/when they
  land — uses `"yes"` / `"no"` strings. Internal storage stays Boolean.
  Convert at every JSON branch and request-param read.
...
- **Rate limit.** Per-user 5-second cache lock on `create` to prevent
  double-clicks and sync bursts. Inline 429 message routes to bracketed-
  link copy "[try again in a moment]".
- **Notifications.** Completion (success + failure) feeds the Phase 16
  pipeline. The user is informed even if the modal was closed.
- **No JavaScript alerts.** All confirms via the standard turbo-frame
  modal pattern. No `data-turbo-confirm`, no `window.confirm`. The
  keep/reject form's `[keep]` submit IS the confirmation step — the user
  explicitly committed by clicking it.
...
- [ ] `ImportJob` model: `belongs_to :channel`, `belongs_to :enqueued_by`,
      enum status, `in_flight`/`for_channel`/`recent` scopes, validations
      from §4.1, callbacks for `started_at`/`completed_at`.
- [ ] `RejectedVideoImport` model: `belongs_to :channel`, `belongs_to
      :rejected_by`, validations from §4.2.
- [ ] `Channel#in_flight_import?` returns true when an `in_flight`
      ImportJob exists for the channel.
- [ ] `Channel` has `has_many :import_jobs`, `has_many
      :rejected_video_imports` with appropriate dependent options.
...
- [ ] `#create` accepts `channel_ids[]`, validates each, refuses channels
      ...
      `ImportJob` per remaining channel, enqueues one `Channel::Import-
      VideosJob` per ImportJob.
- [ ] `#create` is rate-limited per-user via a 5-second cache lock; second
...
- [ ] `Channel::ImportVideosJob` updates status `queued → running →
...
- [ ] `[start import]` is disabled until ≥1 channel is selected.
- [ ] Progress step shows the `=---` indicator per channel and updates
      live via Turbo Stream.
- [ ] Channel show page renders the in-flight badge with a `[view
      progress]` link when an `ImportJob` is running.
...
- [ ] On `ImportJob` `completed` and `failed`, a notification is
      dispatched to `import_job.enqueued_by` via the Phase 16 pipeline.

### Test coverage (spec pyramid sweep per architect rule D)

- [ ] Model specs for `ImportJob` (validations, associations, scopes,
      callbacks, `progress_fraction`, `in_flight?`).
...
- [ ] Service spec for `Channels::VideoImporter`: happy path; channel with
      ...
      error → `FatalError`; partial pagination; previously-imported videos
...
- [ ] Request specs for every endpoint, happy + sad + edge + flaw:
      - `index` happy path; unauthenticated → 401/redirect; no connected
        channels.
      - `create` happy path; rate-limit second-call → 429; in-flight
        channel selection refused; invalid channel id → 422.
      - `show` happy path while running; happy path after completion; not
        found; cross-user access (should still be allowed per single-
        install architecture, but verify).
      - `update` happy path with all kept; happy path with mixed; all
        unchecked → all destroyed + all tombstoned; idempotent re-submit
        (after first submit, candidate set is empty — should be a no-op
        rather than a 500).
...
13. In a `bin/rails console`:
    - `Video.where(channel: <channel>).where(youtube_video_id: <id>).any?`
      → `false`.
    - `RejectedVideoImport.where(channel: <channel>, youtube_video_id:
      <id>).any?` → `true`.
14. Click `[import]` again, tick the same channel, click `[start import]`.
    The new ImportJob runs but does NOT re-import the rejected video.
    Verify in the keep/reject table that the rejected ID is absent.
...
The master agent resolves these with the user before dispatching the rails-
impl lane.
