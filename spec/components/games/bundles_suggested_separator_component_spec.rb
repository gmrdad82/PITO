...
#   - RIGHT column: chevron cluster (`>>>`) in
#     `.bundles-suggested-separator__chevrons`, cueing the
#     transition into the suggested half.
#
# Asserted contract:
...
#   - Inner `.bundles-suggested-separator__chevrons` carries the
#     literal ">>>" run (escaped from `&gt;&gt;&gt;` in ERB).
...
RSpec.describe Games::BundlesSuggestedSeparatorComponent, type: :component do
  ...
  it "renders the chevron cluster ('>>>') in the dedicated chevrons column" do
    render_inline(described_class.new)
